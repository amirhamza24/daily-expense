...
export default function SettingsPage() {
  ...
  const handleClearHistory = async () => {
    ...
    startTransition(async () => {
      ...
      showToast('All transaction logs cleared successfully. Balance reset to zero.', 'success');
    });
  };
      ...
      <div className="flex flex-col gap-6">
        ...
        <GlassCard className="border-white/5 shadow-2xl flex flex-col gap-5">
          <div className="flex items-center gap-2 pb-3 border-b border-slate-200/50 dark:border-white/5">
            ...
            <h3 className="font-bold text-base text-slate-900 dark:text-slate-100">Workspace Theme</h3>
          </div>

          <div className="flex flex-col gap-3">
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
              Choose your preferred color scheme. Your selection is saved and persists across sessions.
            </p>
              ...
              {themeOptions.map((opt) => {
                ...
                return (
                  <button
                    ...
                    `}
                  >
                    <div className={`p-2 rounded-lg ${isActive ? 'bg-violet-500/20' : 'bg-white/5'}`}>
                      {opt.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <span className={`text-sm font-bold block ${isActive ? 'text-violet-600 dark:text-violet-300' : 'text-slate-700 dark:text-slate-300'}`}>
                        {opt.label}
                      </span>
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-violet-500/5 border border-violet-500/15">
              ...
              <span className="text-[10px] text-violet-400 font-semibold tracking-wide uppercase">
                Active: {theme === 'light' ? '☀️ Light Mode' : '🌙 Dark Glow Mode'}
              </span>
            </div>
          </div>
        </GlassCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          ...
          <GlassCard className="border-white/5 shadow-2xl flex flex-col gap-5">
            <div className="flex items-center gap-2 pb-3 border-b border-slate-200/50 dark:border-white/5">
              ...
              <h3 className="font-bold text-base text-slate-900 dark:text-slate-100">Notification Alerts</h3>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 block">Interactive Toast Alerts</span>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 block">Show status banners on CRUD</span>
                </div>
                <button
                  onClick={() => setEnableAlerts(!enableAlerts)}
                  className={`w-10 h-6 rounded-full transition-all duration-300 relative border ${
                    enableAlerts ? 'bg-violet-600/30 border-violet-500/50' : 'bg-white/5 border-white/10'
                  } cursor-pointer`}
                >
                  <span className={`absolute top-0.5 w-4.5 h-4.5 rounded-full bg-slate-100 transition-all duration-300 ${enableAlerts ? 'left-4.5' : 'left-0.5'}`} />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 block">Weekly Digest Email</span>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 block">Summarized stats of expenses</span>
                </div>
                <button
                  onClick={() => setWeeklyDigest(!weeklyDigest)}
                  className={`w-10 h-6 rounded-full transition-all duration-300 relative border ${
                    weeklyDigest ? 'bg-violet-600/30 border-violet-500/50' : 'bg-white/5 border-white/10'
                  } cursor-pointer`}
                >
                  <span className={`absolute top-0.5 w-4.5 h-4.5 rounded-full bg-slate-100 transition-all duration-300 ${weeklyDigest ? 'left-4.5' : 'left-0.5'}`} />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-200/50 dark:border-white/5">
                <div>
                  <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 block">Glassmorphism Panels</span>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 block">Frosted CSS and blurs active</span>
                </div>
          ...
          <GlassCard className="border-white/5 shadow-2xl flex flex-col gap-5">
            <div className="flex items-center gap-2 pb-3 border-b border-slate-200/50 dark:border-white/5">
              ...
              <h3 className="font-bold text-base text-slate-900 dark:text-slate-100">Danger Zone</h3>
            </div>

            <div className="flex flex-col gap-3">
              <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-relaxed">
                Reset your budget limits and clear all transactional records to start fresh. This process is irreversible!
              </p>
